class PersonPageRank < ActiveRecord::Base
  ...
  scope :site_persons_rank, ->(site) {
    person_rank = %{
      SELECT sum(r.rank) as rank, per.name as person_name, s_p.name as site_name 
      FROM   person_page_ranks r 
      JOIN   persons per ON r.person_id = per.id
    }.gsub(/\s+/, ' ').strip
    
    site_pages = %{
      SELECT s.name, p.id FROM sites AS s 
      INNER JOIN pages AS p ON s.id = p.site_id 
      WHERE (s.name = '#{site}')
    }.gsub(/\s+/, ' ').strip
    
    find_by_sql("#{person_rank} JOIN (#{site_pages}) as s_p ON s_p.id = r.page_id GROUP BY person_name")
  }
  
  scope :day_statistic, ->(site, person, start_date, end_date) {
    person_rank = %{
      SELECT sum(r.rank) as rank, per.name as person_name, s_p.name as site_name, s_p.date 
      FROM   person_page_ranks r 
      JOIN   persons per ON r.person_id = per.id
    }.gsub(/\s+/, ' ').strip
